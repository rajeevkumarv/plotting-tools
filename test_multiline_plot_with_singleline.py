...
from plots.lineplot import MultiLinePlot
import numpy as np
...
while(True):
    ...
    for l in range(0,lines):
        a.set_xydata_for_index(np.random.uniform(dataMin,dataMax,size=data_length),np.random.uniform(dataMin,dataMax,size=data_length),l)
    if(s > dataMax ):
        dataMax+=1
        dataMin+=1
